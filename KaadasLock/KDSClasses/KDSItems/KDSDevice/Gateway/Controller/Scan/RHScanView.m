...
- (void)drawScanRect
{
    ...
    //扫码区域Y轴最小坐标
    CGFloat YMinRetangle = KDSScreenHeight > 667 ? 230 : 130;//self.frame.size.height / 2.0 - sizeRetangle.height/2.0 - _viewStyle.centerUpOffset;
...
//            make.bottom.equalTo(self.mas_bottom).offset(-kNavBarHeight-kStatusBarHeight);
            make.height.equalTo(KDSScreenHeight < 667 ? @125 : @175);
        }];
        UIButton * flashlightBtn = [UIButton new];
        [flashlightBtn setImage:[UIImage imageNamed:@"FlashlightImg"] forState:UIControlStateNormal];
        [flashlightBtn addTarget:self action:@selector(flashlightBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        [bottomView addSubview:flashlightBtn];

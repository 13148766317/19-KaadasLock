...
@interface KDSLockedWayViewController ()<UITableViewDelegate,UITableViewDataSource>
...
///服务器上的原始上锁方式
@property(nonatomic,assign)NSUInteger lockingMethodRawData;
///保存按钮
@property(nonatomic,strong)UIButton * saveBtn;
///上锁方式分配值数组
@property (nonatomic,strong) NSArray *lockingMethods;
...
- (void)setUI
{
    ...
    self.currentIndex = self.lock.wifiDevice.lockingMethod;
    self.lockingMethodRawData = self.lock.wifiDevice.lockingMethod;
...
-(void)setDataArray
{
    ...
    [_dataSourceArr addObject:@"自动上锁"];
    [_dataSourceArr addObject:@"定时5秒"];
    [_dataSourceArr addObject:@"定时10秒"];
    [_dataSourceArr addObject:@"定时15秒"];
    [_dataSourceArr addObject:@"关闭自动上锁"];

    self.lockingMethods = @[@(1),@(2),@(3),@(4),@(5)];
}
...
- (void)navBackClick{
    ...
    if (self.lockingMethodRawData == self.currentIndex) {
    ...
    [XMP2PManager sharedXMP2PManager].XMMQTTConnectDevStateBlock = ^(BOOL isCanBeDistributed) {
        if (isCanBeDistributed == YES) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                NSLog(@"MQTT 登录成功");
                [[KDSMQTTManager sharedManager] setLockingMethodWithWf:self.lock.wifiDevice value:(int)self.currentIndex completion:^(NSError * _Nullable error, BOOL success) {
                    [hud hideAnimated:YES];
                    if (success) {
                        [MBProgressHUD showSuccess:Localized(@"上锁方式设置成功")];
                        !weakSelf.lockedModeChangeBlock ?: weakSelf.lockedModeChangeBlock((int)weakSelf.currentIndex);
                    }else{
                        [MBProgressHUD showSuccess:Localized(@"上锁方式设置失败")];
                    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.selectBtn.tag = [self.lockingMethods[indexPath.row] integerValue];
    [cell.selectBtn addTarget:self action:@selector(ringNumClick:) forControlEvents:UIControlEventTouchUpInside];
    if (indexPath.row == [self titleIndexFromLockingMethods:_currentIndex]) {
...
///获取开门方向对应标题序号
-(NSInteger) titleIndexFromLockingMethods:(NSUInteger) hoverAlarmLevel {
    
    __block NSUInteger result = NSNotFound;
    
    [self.lockingMethods enumerateObjectsUsingBlock:^(NSNumber * obj, NSUInteger idx, BOOL * _Nonnull stop) {

//
//  KDSDisplayScreenBrightnessVC.m
//  KaadasLock
//
...
@interface KDSDisplayScreenBrightnessVC ()<UITableViewDelegate,UITableViewDataSource>
...
///显示屏亮度等级
@property (nonatomic,strong) NSArray *screenLightLevels;

@end

@implementation KDSDisplayScreenBrightnessVC
...
- (void)setUI
{
    self.currentIndex = self.lock.wifiDevice.screenLightLevel;
    self.screenLightLevelRawData = self.lock.wifiDevice.screenLightLevel;
...
-(void)setDataArray
{
    ...
    [_dataSourceArr addObject:@"高"];
    [_dataSourceArr addObject:@"中"];
    [_dataSourceArr addObject:@"低"];
    
    self.screenLightLevels = @[@(80),@(50),@(30)];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.selectBtn.tag = [self.screenLightLevels[indexPath.row] integerValue];
...
-(void)navBackClick
{
    ...
    if (self.screenLightLevelRawData == self.currentIndex) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [[KDSMQTTManager sharedManager] setScreenBrightnessWithWf:self.lock.wifiDevice brightness:(int)self.currentIndex completion:^(NSError * _Nullable error, BOOL success) {
                    ...
                    if (success) {
                        ...
                        !weakSelf.screenBrightnessChangeBlock ?: weakSelf.screenBrightnessChangeBlock((int)weakSelf.currentIndex);
                    }else{
...
-(NSInteger) titleIndexFromScreenLightLevel:(NSUInteger) hoverAlarmLevel {
    ...
    [self.screenLightLevels enumerateObjectsUsingBlock:^(NSNumber * obj, NSUInteger idx, BOOL * _Nonnull stop) {
